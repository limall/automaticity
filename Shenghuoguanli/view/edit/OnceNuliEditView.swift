                    ...
                    if let _ = Double(rate){
                        if title != ""{
                            if Save.sharedInstance.getNuliIndexWithExcept(title: title, except: title_origin) == -1 && Save.sharedInstance.getFangsongIndexWithExcept(title: title, except: title_origin) == -1{
                                Action.sharedInstance.edit(oldTitile: title_origin, newTitle: title, rate: rate, type: NULITYPE_ONCE)
                                title_origin=title
...
struct OnceNuliEditView_Previews: PreviewProvider {
    static var previews: some View {
        OnceNuliEditView(index: 0)
    }
}
