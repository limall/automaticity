//
//  ContinueFangsongEditView.swift
//  Shenghuoguanli
//
...
struct ContinueFangsongEditView: View {
    let index:Int
    ...
    var body: some View {
                    ...
                    if let _ = Double(rate){
                        if title != ""{
                            if Save.sharedInstance.getNuliIndexWithExcept(title: title, except: title_origin) == -1 && Save.sharedInstance.getFangsongIndexWithExcept(title: title, except: title_origin) == -1{
                                Action.sharedInstance.edit(oldTitile: title_origin, newTitle: title, rate: rate, type: FANGSONGTYPE_CONTINUE)
                                title_origin=title
        ...
        .onAppear{
            ...
            title=title_origin
            rate=ContentView.dataStore.fangsongDatas[ContentView.dataStore.getFangsongIndex(title: title)].rate
        }
    }
}

struct ContinueFangsongEditView_Previews: PreviewProvider {
    static var previews: some View {
        ContinueFangsongEditView(index: 0)
    }
}
