//
//  CreateContinueFangsongView.swift
//  Shenghuoguanli
//
...
struct CreateContinueFangsongView: View {
    @State var title=""
    @State var rate=""
                ...
                if let _ = Double(rate){
                    if title != ""{
                        if Save.sharedInstance.getNuliIndex(title: title) == -1 && Save.sharedInstance.getFangsongIndex(title: title) == -1{
                            Action.sharedInstance.createContinueFangsong(title: title, rate: rate)
...
struct CreateContinueFangsongView_Previews: PreviewProvider {
