...
struct CreateOnceFangsongView: View {
    @State var title=""
    @State var rate=""
                ...
                if let _ = Double(rate){
                    if title != ""{
                        if Save.sharedInstance.getNuliIndex(title: title) == -1 && Save.sharedInstance.getFangsongIndex(title: title) == -1{
                            Action.sharedInstance.createOnceFangsong(title: title, rate: rate)
...
struct CreateOnceFangsongView_Previews: PreviewProvider {
