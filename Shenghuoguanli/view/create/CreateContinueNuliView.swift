...
struct CreateContinueNuliView: View {
    @State var title=""
    @State var rate=""
    ...
    var body: some View {
        VStack{
            ...
            VStack(spacing:1){
                TextField("请输入比率，只能是数字", text: $rate)
                    .font(.system(size: screen.width*0.06))
                    .frame(width:screen.width*0.74)
                ...
                if let _ = Double(rate){
                    if title != ""{
                        if Save.sharedInstance.getNuliIndex(title: title) == -1 && Save.sharedInstance.getFangsongIndex(title: title) == -1{
                            Action.sharedInstance.createContinueNuli(title: title, rate: rate)
...
struct CreateContinueNuliView_Previews: PreviewProvider {
